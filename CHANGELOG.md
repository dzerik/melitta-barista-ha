...
- **Translations that were quietly English are now translated.** An audit across every string family found 4182 values sitting on the English source and 3909 keys missing outright. In Russian the sensors read "Total beverages", "Hot water" and "Rinse cycles"; in Dutch the entire BLE repair and re-pair flow — the instructions a user follows precisely when their machine has stopped working — was English. 28 languages were brought up across the integration's entity strings, the served strings and the panel bundles, and a cross-family seam was closed: several languages translated "open the System tab" but left the tab's name in English, pointing the reader at a tab that does not exist in their language. Placeholder, newline and markup structure was verified byte-for-byte against English across all 41948 localized strings. New tests fail the build when a locale value is byte-identical to English outside an explicit, justified allowlist — the pin that would have caught this years ago.
...
- Entity *names* stay English on purpose: Home Assistant derives `entity_id` from them and the UI Contract publishes the suffix, so translating them would hand every language a different `entity_id` and break dashboards and automations on language change. They are the largest block on the allowlist and are marked as such.
- Matching clients: Lovelace card `2.11.1`, app `3.1.4` — both carry the same translation pass and their own regression pins.
