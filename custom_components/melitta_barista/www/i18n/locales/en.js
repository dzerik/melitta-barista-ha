/**
 * English translation strings.
 *
 * English is the source of truth; other locales fall back here when a key
 * is missing. Adding a new key here without a counterpart in other locales
 * is fine — the runtime resolver falls through to English transparently.
 *
 * Parity is enforced by tests/test_i18n_parity.py — every locale must
 * carry the same key set.
 */

export default {
  "panel.title": "Melitta Barista",
  "panel.no_entries": "No coffee machines configured for this integration.",

  "tabs.system": "System",
  "tabs.recipes": "Recipes",
  "tabs.beans": "Beans",
  "tabs.additives": "Add-ins",
  "tabs.producers": "Producers",
  "tabs.sommelier": "Sommelier",

  "system.subtabs.status": "Status",
  "system.subtabs.settings": "Settings",
  "system.subtabs.diagnostics": "Diagnostics",
  "system.subtabs.recipes": "Machine recipes",

  "common.loading": "Loading…",
  "common.error": "Error",
  "common.save": "Save",
  "common.cancel": "Cancel",
  "common.delete": "Delete",
  "common.delete_confirm": "Delete this item?",
  "common.add": "Add",
  "common.edit": "Edit",
  "common.refresh": "Refresh",
  "common.yes": "Yes",
  "common.no": "No",
  "common.unknown": "Unknown",
  "common.never": "Never",
  "common.empty": "Empty",
  "common.confirm": "Confirm",
  "confirm.delete.title": "Delete item",
  "confirm.delete.confirm": "Delete",

  "status.title": "Machine status",
  "status.ble_state": "BLE connection",
  "status.connected": "Connected",
  "status.disconnected": "Disconnected",
  "status.machine_state": "Machine state",
  "status.process": "Process",
  "status.manipulation": "Prompt",
  "status.firmware": "Firmware",
  "status.model": "Model",
  "status.family": "Family",
  "status.slots": "MyCoffee slots",
  "status.machine_type": "Machine type",
  "status.dis": "Device information",
  "status.cup_total": "Total cups",
  "status.cup_by_recipe": "Cups by recipe",
  "status.no_status": "No status received yet.",
  "status.last_update": "Last status update",
  "status.profile_active": "Active profile",
  "status.selected_recipe": "Selected recipe",

  "diag.title": "Diagnostics",
  "diag.recent_errors": "Recent errors",
  "diag.recent_frames": "Recent BLE frames",
  "diag.no_errors": "No errors recorded.",
  "diag.no_frames": "No frames recorded yet.",
  "diag.address": "BLE address",
  "diag.brand": "Brand",
  "diag.proxy": "Transport",
  "diag.proxy_local": "Local BlueZ adapter",
  "diag.proxy_remote": "ESPHome BLE proxy",
  "diag.poll_interval": "Polling interval",
  "diag.handshake": "Last handshake",
  "diag.clear": "Clear log",

  "recipes.title": "Recipes & DirectKey",
  "recipes.base_recipes": "Base recipes",
  "recipes.directkey": "DirectKey profiles",
  "recipes.id": "ID",
  "recipes.name": "Name",
  "recipes.process": "Process",
  "recipes.intensity": "Intensity",
  "recipes.aroma": "Aroma",
  "recipes.portion": "Portion (ml)",
  "recipes.temperature": "Temperature",
  "recipes.shots": "Shots",
  "recipes.profile": "Profile",
  "recipes.category": "Category",
  "recipes.reset_default": "Reset to default",
  "recipes.editor_title": "Profile recipes (DirectKey)",
  "recipes.editor_hint": "Per-profile drink presets stored on the machine. Click a drink to edit it.",
  "recipes.disconnected": "Machine is disconnected — recipes can't be edited until it reconnects.",
  "recipes.no_data": "No DirectKey recipes read from the machine yet.",
  "recipes.component1": "Component 1",
  "recipes.component2": "Component 2",
  "recipes.edit_title": "Edit recipe — {category}",
  "recipes.empty_slot": "Not read from the machine yet — saving writes a new recipe.",
  "recipes.save_success": "Recipe saved: {category}",
  "recipes.save_failed": "Saving failed",
  "recipes.reset_success": "Recipe reset to factory defaults: {category}",
  "recipes.reset_failed": "Reset failed",
  "recipes.reset_confirm_title": "Reset recipe",
  "recipes.reset_confirm": "Reset this recipe to factory defaults? Your customization will be lost.",
  "recipes.no_entity": "No machine button entity found — cannot call the service.",
  "recipes.cat.espresso": "Espresso",
  "recipes.cat.cafe_creme": "Café crème",
  "recipes.cat.cappuccino": "Cappuccino",
  "recipes.cat.latte_macchiato": "Latte macchiato",
  "recipes.cat.milk_froth": "Milk froth",
  "recipes.cat.milk": "Warm milk",
  "recipes.cat.water": "Hot water",
  "recipes.opt.none": "None",
  "recipes.opt.coffee": "Coffee",
  "recipes.opt.milk": "Milk",
  "recipes.opt.water": "Water",
  "recipes.opt.very_mild": "Very mild",
  "recipes.opt.mild": "Mild",
  "recipes.opt.medium": "Medium",
  "recipes.opt.strong": "Strong",
  "recipes.opt.very_strong": "Very strong",
  "recipes.opt.standard": "Standard",
  "recipes.opt.intense": "Intense",
  "recipes.opt.cold": "Cold",
  "recipes.opt.normal": "Normal",
  "recipes.opt.high": "High",
  "recipes.opt.one": "1 shot",
  "recipes.opt.two": "2 shots",
  "recipes.opt.three": "3 shots",

  "beans.title": "Coffee beans & producers",
  "beans.producers": "Producers",
  "beans.beans": "Beans",
  "beans.add_producer": "Add producer",
  "beans.add_bean": "Add bean",
  "beans.producer_name": "Producer",
  "beans.bean_name": "Name",
  "beans.roast": "Roast",
  "beans.origin": "Origin",
  "beans.varietal": "Varietal",
  "beans.notes": "Notes",
  "beans.recommended_brewing": "Recommended brewing",
  "beans.brewing_label": "Brewing",
  "beans.hopper": "Hopper",
  "beans.hopper_left": "Left hopper",
  "beans.hopper_right": "Right hopper",
  "beans.autofill": "Fill from LLM",
  "beans.autofill_running": "Asking the assistant…",
  "beans.autofill.debug": "Autofill debug info",
  "beans.no_beans": "No beans added yet.",
  "beans.no_producers": "No producers yet — add one first.",
  "beans.no_producers_go_to_tab": "Add a producer in the Producers tab first.",
  "beans.hopper.assigned": "✓ Hopper {hopper}: {bean}",
  "beans.hopper.mismatch": "Save not confirmed: WS returned OK, but hopper {hopper} contains \"{actual}\" instead of \"{expected}\" after reload. Check HA logs.",
  "beans.hopper.failed": "Assign to hopper {hopper} failed: {error}",

  "additives.title": "Add-ins",
  "additives.syrups": "Syrups",
  "additives.toppings": "Toppings",
  "additives.milk": "Milk types",
  "additives.add": "Add",
  "additives.name": "Name",
  "additives.brand": "Brand",
  "additives.notes": "Notes",
  "additives.empty_syrups": "No syrups configured.",
  "additives.empty_toppings": "No toppings configured.",
  "additives.empty_milk": "No milk types configured.",
  "additives.in_stock": "In stock",
  "additives.out_of_stock": "Out of stock",
  "additives.toggle_stock_failed": "Failed to update stock state.",
  "additives.producer": "Producer",
  "additives.producer_none": "(none)",
  "additives.variant": "Variant",
  "additives.flavor_notes": "Flavor notes",
  "additives.flavor_notes_add": "Add a note…",
  "additives.composition": "Composition",
  "additives.attributes": "Attributes",
  "additives.attr.vegan": "Vegan",
  "additives.attr.sugar_free": "Sugar-free",
  "additives.attr.lactose_free": "Lactose-free",
  "additives.attr.gluten_free": "Gluten-free",
  "additives.attr.nut_free": "Nut-free",
  "additives.fill_from_llm": "Fill from LLM",
  "additives.fill_failed": "LLM autofill failed.",
  "additives.fill_needs_producer_and_name": "Select a producer and enter a name to use Fill from LLM.",

  "producers.title": "Producers",
  "producers.add": "Add",
  "producers.empty": "No producers yet.",
  "producers.name": "Name",
  "producers.country": "Country",
  "producers.website": "Website",
  "producers.notes": "Notes",
  "producers.confirm_delete": "Delete this producer? Beans and additives that reference it will lose the link.",
  "producers.save_failed": "Could not save producer",
  "producers.delete_failed": "Could not delete producer",
  "producers.load_failed": "Could not load producers",

  "sommelier.title": "AI Sommelier",
  "sommelier.prompt": "Describe what you'd like to drink",
  "sommelier.generate": "Generate",
  "sommelier.brew_this": "Brew this",
  "sommelier.context_beans": "Beans in hopper",
  "sommelier.context_milk": "Milk available",
  "sommelier.context_extras": "Add-ins available",
  "sommelier.last_recipe": "Latest recipe",
  "sommelier.no_recipe": "Ask the sommelier to design a drink.",
  "sommelier.brewing": "Sending brew command…",
  "sommelier.brew_ok": "Brew started.",
  "sommelier.brew_failed": "Brew failed.",
  "sommelier.recipe_not_found": "Recipe not found",
  "sommelier.favorites_button": "★ Favorites",
  "sommelier.rate_failed": "Could not save rating",

  "presets.label": "Preset",
  "presets.none": "(none)",
  "presets.save_as": "Save as preset",
  "presets.manage": "Manage",
  "presets.modal_title": "Manage presets",
  "presets.empty": "No presets yet.",
  "presets.name": "Name",
  "presets.description": "Description",
  "presets.save": "Save",
  "presets.delete": "Delete",
  "presets.delete_confirm": "Delete this preset?",
  "presets.rename": "Rename",
  "presets.save_failed": "Could not save preset",
  "presets.delete_failed": "Could not delete preset",
  "presets.load_failed": "Could not load presets",
  "presets.applied_toast": "Preset \"{name}\" applied",
  "presets.builtin_badge": "built-in",
  "presets.system_readonly_error": "Built-in presets cannot be modified.",
  "presets.system.morning": "Morning",
  "presets.system.after_lunch": "After lunch",
  "presets.system.work": "Work",
  "presets.system.guests": "Guests",
  "presets.bind_to_profile": "Bind to profile {n}",

  "profile.label": "Profile {n}",
  "profile.tooltip": "Active machine profile. Lists are filtered to this profile and shared entries.",

  "common.validation_errors": "Validation errors:",

  "favorites.modal_title": "Saved favorites",
  "favorites.load_failed": "Failed to load favorites",
  "favorites.rate_failed": "Could not save rating",
  "favorites.update_failed": "Could not save changes",
  "favorites.delete_failed": "Could not delete",
  "favorites.empty": "No favorites saved yet.",
  "favorites.brew": "Brew",
  "favorites.note_label": "Note",
  "favorites.note_placeholder": "Tasting note (optional)…",
  "favorites.note_needs_rating": "Set a rating first to add a tasting note.",
  "favorites.name_placeholder": "Name",
  "favorites.description_placeholder": "Description (optional)…",
  "favorites.brewed_count": "Brewed {n}×",
  "favorites.delete.title": "Delete favorite",
  "favorites.delete.confirm": "Delete \"{name}\"? This cannot be undone.",

  "sommelier.history_button": "History",

  "history.modal_title": "Generation history",
  "history.load_failed": "Failed to load history",
  "history.rate_failed": "Could not save rating",
  "history.empty": "No generation history yet.",
  "history.load_more": "Load more",
  "history.brew_again": "Brew this",
  "history.brewed": "Brewed",
  "history.note_label": "Note",
  "history.recipe_count": "{n} recipes",
  "history.mode.surprise_me": "Surprise me",
  "history.mode.custom": "Custom",
  "history.clear.button": "Clear history",
  "history.clear.title": "Clear history",
  "history.clear.confirm": "Delete all generation sessions? Sessions linked to favorites are kept.",
  "history.clear.confirm_button": "Clear",
  "history.clear.done": "{n} sessions removed.",
  "history.clear.failed": "Could not clear history",

  "wizard.title": "Brew wizard",
  "wizard.step_of": "Step {n} of {m}",
  "wizard.step.cup": "Place a {cup} (~{ml} ml) under the spout",
  "wizard.step.done": "Done",
  "wizard.step.machine": "Machine pour",
  "wizard.step.machine_n": "Machine pour {n} of {m}",
  "wizard.machine.start": "Start this pour",
  "wizard.machine.start_full": "Start brewing",
  "wizard.machine.estimated": "~{sec}s",
  "wizard.machine.waiting": "Waiting for the machine...",
  "wizard.machine.im_done": "It's done, continue",
  "wizard.machine.failed": "Brew failed",
  "wizard.machine.retry": "Retry",
  "wizard.machine.skip": "Skip (brew manually)",
  "wizard.machine.during_hint": "While it brews:",
  "settings.llm_timeout": "LLM timeout (seconds)",
  "settings.llm_timeout_help": "How long to wait for the AI agent. Increase (e.g. 180–300) for slow local models — the Sommelier's prompt is large and local models can need minutes to process it.",
  "settings.llm_timeout_invalid": "Enter a value between 10 and 600 seconds.",
  "settings.compact_prompt": "Compact prompt",
  "settings.compact_prompt_help": "Send the Sommelier a much shorter prompt. Recommended for slow local models — the beans, inventory, machine limits and your existing recipes are kept, verbose guidance is trimmed.",
  "settings.compact_prompt_label": "Use compact prompt",
  "sommelier.err.no_llm_agent": "No AI (LLM) integration is set up in Home Assistant — the Sommelier needs one.",
  "sommelier.err.no_llm_agent_selected": "Select an AI agent in Settings → AI agent.",
  "sommelier.err.llm_agent_missing": "The selected AI agent is gone — pick another in Settings.",
  "wizard.machine.prompt_generic": "machine prompt",
  "wizard.machine.prompt": "The machine asks for confirmation: {prompt}",
  "wizard.machine.confirm": "Confirm",
  "wizard.machine.confirm_failed": "Could not confirm the prompt",
  "wizard.machine.confirm_manual": "Confirm the prompt on the machine display.",
  "wizard.finish.title": "All done!",
  "wizard.finish.message": "Enjoy your coffee!",
  "wizard.finish.button": "Done",
  "wizard.close.title": "Close the wizard?",
  "wizard.close.message": "The machine keeps brewing on its own. Your position is saved — reopening the wizard resumes right here.",
  "wizard.close.stay": "Stay",
  "wizard.close.leave": "Close",
  "wizard.resumed": "Resumed where you left off.",
  "wizard.restart": "Start over",

  "brewing.unsupported_tooltip": "This machine cannot accept custom recipes — recipe is print-only.",
  "brewing.unsupported_note": "Custom recipe writes are not supported on this machine family. View the recipe and brew it manually via the machine's own recipe selector.",
  "brewing.unsupported_error": "This machine does not support custom recipe writes.",

  "sommelier.constraints_heading": "Constraints and mood",
  "sommelier.cup_size": "Cup size",
  "sommelier.mood_label": "Mood (multi-select)",
  "sommelier.occasion_label": "Occasion (suggested from time of day)",
  "sommelier.temperature_label": "Temperature",
  "sommelier.caffeine_label": "Caffeine",
  "sommelier.dietary_label": "Dietary (multi-select)",
  "sommelier.addins_heading": "Available add-ins (multi-select)",
  "sommelier.section_syrups": "Syrups",
  "sommelier.section_toppings": "Toppings",
  "sommelier.section_milk": "Milk",
  "sommelier.addin_unconfigured": "— not configured under Add-ins",
  "sommelier.machine_label": "Machine:",
  "sommelier.why": "Why?",
  "sommelier.favorited_toast": "★ Added to favorites",
  "sommelier.fav_in": "Favorited",
  "sommelier.fav_add": "Add to favorites",
  "sommelier.favorite_failed": "Failed to add to favorites",
  "sommelier.addins_load_failed": "Failed to load add-ins",

  "sommelier.cup.espresso_cup": "Espresso cup",
  "sommelier.cup.cup": "Cup",
  "sommelier.cup.mug": "Mug",
  "sommelier.cup.tall_glass": "Tall glass",
  "sommelier.cup.travel": "Travel mug",

  "sommelier.mood.energizing": "Energizing",
  "sommelier.mood.relaxing": "Relaxing",
  "sommelier.mood.dessert": "Dessert",
  "sommelier.mood.classic": "Classic",

  "sommelier.occasion.morning": "Morning",
  "sommelier.occasion.after_lunch": "After lunch",
  "sommelier.occasion.guests": "Guests",
  "sommelier.occasion.romantic": "Romantic",
  "sommelier.occasion.work": "Work",

  "sommelier.temp.auto": "Auto",
  "sommelier.temp.hot": "Hot",
  "sommelier.temp.iced": "Iced",
  "sommelier.gate.unsupported": "Not supported by this machine",

  "sommelier.caffeine.regular": "Regular",
  "sommelier.caffeine.low": "Low",
  "sommelier.caffeine.decaf_evening": "Decaf (evening)",

  "sommelier.diet.no_sugar": "No sugar",
  "sommelier.diet.lactose_free": "Lactose-free",
  "sommelier.diet.low_calorie": "Low calorie",
  "sommelier.diet.vegan": "Vegan",

  "modal.add_bean": "Add bean",
  "modal.edit_bean": "Edit bean",
  "modal.add_producer": "Add producer",
  "modal.edit_producer": "Edit producer",
  "modal.add_additive": "Add add-in",
  "modal.edit_additive": "Edit add-in",
  "modal.type": "Type",
  "modal.type.syrup": "Syrup",
  "modal.type.topping": "Topping",
  "modal.type.milk": "Milk",

  "tags.title": "Flavor notes",
  "tags.add_placeholder": "Add tag…",
  "tags.add_button": "Add tag",

  "hopper.title": "Hopper assignment",
  "hopper.left": "Hopper 1 (left)",
  "hopper.right": "Hopper 2 (right)",
  "hopper.unassigned": "— not set —",
  "hopper.assigned": "Assigned",

  "settings.title": "Settings",
  "settings.llm_agent": "LLM model",
  "settings.llm_help": "Conversation agent used for Sommelier and bean autofill.",
  "settings.prompts": "Prompt templates",
  "settings.prompt_default": "Default",
  "settings.prompt_reset": "Reset to default",
  "settings.prompt_save": "Save",
  "settings.saved": "Saved.",
  "settings.reset_done": "Reset to default.",
  "settings.help_title": "How prompt templates work",
  "settings.help_placeholders": "Placeholders",
  "settings.help_no_placeholders": "No placeholders for this slot.",
  "settings.help_syntax": "Syntax",
  "settings.help_syntax_text":
    "Use Python str.format syntax: {name} for a value, {name!r} to wrap it in quotes. " +
    "Unknown placeholders pass through literally so you can see them in the LLM reply.",
  "settings.help_schema":
    "When a JSON Schema is shown below, it is automatically appended to your template " +
    "before the request is sent. The model is instructed to reply with strict JSON " +
    "matching that schema; replies are validated server-side and one auto-retry is " +
    "performed if validation fails.",
  "settings.help_smartchain":
    "If the SmartChain integration is installed and a SmartChain agent is selected, " +
    "the request is routed through its native structured-output API (no parsing — " +
    "the provider returns a strict object). Otherwise the text+validation path runs.",
  "settings.preview": "Preview assembled prompt",
  "settings.preview_title": "Assembled prompt",
  "settings.preview_loading": "Building preview…",

  "diag.llm_calls": "Recent LLM calls",
  "diag.no_llm_calls": "No LLM calls recorded yet.",
  "diag.llm_show_prompt": "Show prompt",
  "diag.llm_show_response": "Show response",

  // Backup & restore (System -> Settings subtab).
  "backup.title": "Backup & restore",
  "backup.help": "Export the whole Sommelier configuration — beans, hoppers, milk types, add-ins, producers, profiles, favorites, presets and prompt templates — as one JSON file, and import it back here or on another installation.",
  "backup.privacy_note": "The export contains your profile names, dietary preferences, prompt templates and — with history — the weather recorded at generation time; review it before sharing.",
  "backup.include_history": "Include generation history",
  "backup.include_history_help": "Adds every Sommelier session and its recipes to the file. Importing always clears the local history, whether or not the file carries one.",
  "backup.include_install_specific": "Take LLM agent and weather entity from the backup",
  "backup.include_install_specific_help": "Off: keep this installation's own LLM agent and weather entity; the backup's values are ignored. On: take them from the backup as well.",
  "backup.export": "Export…",
  "backup.export_done": "Export downloaded.",
  "backup.export_large": "This export is larger than 3 MB, so it cannot be imported through this page. Put it in the backups folder and restore it from the snapshot list instead.",
  "backup.import": "Import…",
  "backup.import_parse_failed": "That file is not valid JSON.",
  "backup.import_too_large": "That file is larger than 3 MB. Put it in the backups folder and restore it from the snapshot list instead.",
  "backup.import_done": "Imported {count} entries.",
  "backup.snapshots": "Snapshots",
  "backup.snapshots_help": "A snapshot of the current configuration is saved before every import, so a replace can always be undone. Automatic snapshots are pruned over time; ones you keep yourself are not.",
  "backup.snapshots_empty": "No snapshots yet.",
  "backup.snapshots_unavailable": "The snapshot list could not be read.",
  "backup.snapshot_auto": "Automatic",
  "backup.snapshot_manual": "Kept",
  "backup.snapshot_download": "Download",
  "backup.snapshot_restore": "Restore",
  "backup.snapshot_delete": "Delete",
  "backup.restore_done": "Snapshot restored.",
  "backup.delete_done": "Snapshot deleted.",

  "confirm.import.title": "Replace the Sommelier configuration?",
  "confirm.import.message": "This replaces ALL Sommelier data — beans, hoppers, milk types, add-ins, producers, profiles, favorites, presets and prompt templates — with the contents of «{file}». Your generation history will be cleared. A snapshot of the current configuration is saved first, so you can undo this from the Snapshots list below.",
  "confirm.import.confirm": "Replace everything",
  "confirm.restore.title": "Restore this snapshot?",
  "confirm.restore.message": "This replaces ALL Sommelier data with the contents of «{name}». A snapshot of the current configuration is saved first.",
  "confirm.snapshot_delete.title": "Delete this snapshot?",
  "confirm.snapshot_delete.message": "«{name}» will be removed from disk. This cannot be undone.",
};
