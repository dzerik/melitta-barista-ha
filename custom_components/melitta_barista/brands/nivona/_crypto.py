"""Nivona-specific cryptographic constants.

The runtime RC4 key bootstraps the vendor stream cipher after the
customer-key exchange, and the 256-byte HU table feeds the 2-round
CRC fold used for handshake verification. Both are brand-wide — same
values across every family — so they live one level above the per-family
tables.
"""
